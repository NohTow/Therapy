...
sorted_lists_explainer = sorted(zip(model.coef_[0], feature_names), reverse=True, key=lambda x: x[0])

# For binary case, the second class weights are just the opposite of the first one, so we take the smallest weights as highest score for it
top_words = {}
for i, duple in enumerate([sorted_lists_explainer[:200], sorted_lists_explainer[-200:]]):
    top_words[i] = {}
    for coeff, word in duple:
        top_words[i][word] = coeff
json.dump(top_words, open("./top_words/amazon_polarity.json", 'w'))
